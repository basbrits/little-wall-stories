...
function checkout() {
  ...
  const orderLines = state.cart
    .map((item) => `${item.quantity}× ${products[item.id].name} — ${currency.format(products[item.id].price * item.quantity)}`)
    .join('\n');
  ...
  window.location.href = `mailto:hello@littlewallstories.com?subject=${subject}&body=${body}`;
}
